...
class ParamsWrapper : public QObject
{
    ...
    QString getGridDimensions() {return QString("%1 x %2 x %3").arg(prms->GridXTotal).arg(prms->GridY).arg(prms->GridZ);}
    ...
    double getDPPhi() {return std::atan(prms->DP_tan_phi)*180/SimParams3D::pi;}
