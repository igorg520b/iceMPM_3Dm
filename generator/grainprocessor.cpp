...
void GrainProcessor::generate_cone(float diameter, float top, float angle, float height, int n)
{
    ...
    GenerateBlock(diameter, height, diameter, n*vBlock/volume);
...
void GrainProcessor::GenerateBlock(float bx, float by, float bz, int n)
{
...
void GrainProcessor::Write_HDF5(std::string fileName)
{
    for(int k=0;k<3;k++)
    {
        coordinates[k].resize(buffer.size());
        for(int i=0;i<buffer.size();i++)
        {
            coordinates[k][i] = buffer[i][k];
        }
    }
    ...
    H5::DataSet dataset_z = file.createDataSet("z", H5::PredType::NATIVE_DOUBLE, dataspace_points_grains, proplist1);
    dataset_z.write(coordinates[2].data(), H5::PredType::NATIVE_DOUBLE);

    hsize_t att_dim = 1;
    H5::DataSpace att_dspace(1, &att_dim);
    H5::Attribute att_volume = dataset_grainids.createAttribute("volume", H5::PredType::NATIVE_DOUBLE, att_dspace);
